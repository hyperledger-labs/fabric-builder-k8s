...
func WaitForChaincodeJob(
	...
	_, err = waitForJobTermination(ctx, logger, client, job.Name, job.Namespace)
	if err != nil {
		return fmt.Errorf(
	...
	// 		return true, nil
	// 	} else if (c.Type == batchv1.JobFailed || c.Type == batchv1.JobFailureTarget) && c.Status == "True" {
	// 		return true, fmt.Errorf("job %s/%s failed for reason %s: %s", namespace, jobName, c.Reason, c.Message)
	// 	} else {
	// 		logger.Debugf("Status type %s for job %s/%s: %v", c.Type, namespace, jobName, c)
	// 	}
	// }
